...
define("BASE_URL", "http://localhost/project_ssip/");
...
function kategori($kategori_id = false)
{
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        if ($kategori_id == $row['kategori_id']) {
            $string .= "<li><a class='active' href='" . BASE_URL . "$row[kategori_id]/$kategori.html'>$row[kategori]</a></li>";
        } else {
...
{
    if ($level != "admin") {
        $admin_pages = array("kategori", "barang", "kota", "user");
...
function pagination($query, $data_per_halaman, $pagination, $url)
{
    ...
    if ($pagination < $batasAkhirPagination) {
        $next = $pagination + 1;
